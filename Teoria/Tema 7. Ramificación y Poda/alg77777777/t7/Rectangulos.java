// PROBLEMA 3: COLOCACIÓN ÓPTIMA DE RECTÁNGULOS
...
{    
	...
	{
		int n = Integer.parseInt(args[0]); //tamaño tablero 
...
/* Concrección del nodo en este Problema 3 */
{
	private int[][] tablero; //tablero
	private ArrayList<Pieza> piezas; //rectángulos a colocar
	...
	{ 
		...
		// EJEMPLO de 4 PIEZAS (4 Rectángulos)
	...
	/* construye todos los demás nodos no raiz */
	{
	...
	private int[][] intentarPosNuevaPieza(int x,int y,Orientacion orientacion) 
	{
		...
		if (insertarNuevaPieza(x,y,orientacion,nuevoTablero,piezas.get(profundidad)))
			return nuevoTablero;
		else return null; // nodo no válido
	}
	...
	private boolean insertarNuevaPieza(int x,int y,Orientacion orientacion,int[][] nuevoTablero, Pieza pieza)
	{
		...
		// comprobar si la pieza está en otra pieza (se solapa)
	...
	/* cuando se ciolocan todas las piezas es una solución */
	{
